package com.jme3.math;

/**
 * Credits for this class goes to user aioobe on stackoverflow.com
 * Source: http://stackoverflow.com/questions/4454630/j2me-calculate-the-the-distance-between-2-latitude-and-longitude
 * <br>
 * Copied from org.bukkit.craftbukkit.TrigMath to get around package versioning issues without losing efficiency.
 * Please guys, stop hiding the good stuff when this should definitely be part of the API!
 */
public class CommonTrigMath {
    static final float sq2p1 = 2.414213562373095048802e0f;
    static final float sq2m1 = .414213562373095048802e0f;
    static final float p4 = .161536412982230228262e2f;
    static final float p3 = .26842548195503973794141e3f;
    static final float p2 = .11530293515404850115428136e4f;
    static final float p1 = .178040631643319697105464587e4f;
    static final float p0 = .89678597403663861959987488e3f;
    static final float q4 = .5895697050844462222791e2f;
    static final float q3 = .536265374031215315104235e3f;
    static final float q2 = .16667838148816337184521798e4f;
    static final float q1 = .207933497444540981287275926e4f;
    static final float q0 = .89678597403663861962481162e3f;
    static final float PIO2 = 1.5707963267948966135E0f;

    private static float mxatan(float arg) {
        float argsq = arg * arg, value;

        value = ((((p4 * argsq + p3) * argsq + p2) * argsq + p1) * argsq + p0);
        value = value / (((((argsq + q4) * argsq + q3) * argsq + q2) * argsq + q1) * argsq + q0);
        return value * arg;
    }

    private static float msatan(float arg) {
        return arg < sq2m1 ? mxatan(arg)
                : arg > sq2p1 ? PIO2 - mxatan(1 / arg)
                : PIO2 / 2 + mxatan((arg - 1) / (arg + 1));
    }

    public static float atan(float arg) {
        return arg > 0 ? msatan(arg) : -msatan(-arg);
    }

    public static float atan2(float arg1, float arg2) {
        if (arg1 + arg2 == arg1)
            return arg1 >= 0 ? PIO2 : -PIO2;
        arg1 = atan(arg1 / arg2);
        return arg2 < 0 ? arg1 <= 0 ? arg1 + FastMath.PI : arg1 - FastMath.PI : arg1;
    }
}