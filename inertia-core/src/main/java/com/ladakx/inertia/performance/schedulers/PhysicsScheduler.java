package com.ladakx.inertia.performance.schedulers;

import com.ladakx.inertia.InertiaPlugin;
import com.ladakx.inertia.bullet.space.MinecraftSpace;
import com.ladakx.inertia.bullet.space.SpaceManager;

import java.util.concurrent.TimeUnit;

/**
 * Scheduler for handling physics tasks at a fixed rate.
 */
public class PhysicsScheduler extends AbstractScheduler {

    private final SpaceManager spaceManager;
    private final int tickRate;

    /**
     * Constructor to initialize the PhysicsScheduler.
     * @param spaceManager The SpaceManager instance to manage Minecraft spaces.
     */
    public PhysicsScheduler(SpaceManager spaceManager) {
        this.spaceManager = spaceManager;
        this.tickRate = InertiaPlugin.getPConfig().PHYSICS.tickRate;

        start();
    }

    /**
     * Start the physics scheduler.
     */
    @Override
    public void start() {
        // Check if simulation is enabled
        if (!InertiaPlugin.getPConfig().PHYSICS.enable) return;

        // Schedule the task to run at a fixed rate
        timerService.scheduleAtFixedRate(this::process, 1000, 1000 / tickRate, TimeUnit.MILLISECONDS);
    }

    /**
     * Process the physics tasks.
     */
    @Override
    protected void process() {
        for (MinecraftSpace space : spaceManager.getAll()) {
            space.stepPhysics();
        }
    }

    /**
     * Stop the physics scheduler.
     */
    @Override
    public void stop() {
        if (!timerService.isShutdown()) {
            timerService.shutdown();
        }
    }
}