...
final class BuoyancyNarrowPhase {
    ...
    BuoyancyNarrowPhase(PhysicsWorld physicsWorld) {
        this.physicsWorld = Objects.requireNonNull(physicsWorld, "physicsWorld");
    }

    void applyForces(ConstBodyLockInterface lockInterface, float deltaTime, BuoyancyDataStore dataStore) {
        ...
        for (int i = 0; i < dataStore.getCount(); ++i) {
            ...
            try (BodyLockWrite lock = new BodyLockWrite(lockInterface, bodyId)) {
                ...
                FluidType type = dataStore.fluidTypes[i];
                float buoyancyFactor = type == FluidType.LAVA ? 2.5f : 1.1f;
                float linearDrag = type == FluidType.LAVA ? 5.0f : 0.5f;
                float angularDrag = type == FluidType.LAVA ? 2.0f : 0.05f;
