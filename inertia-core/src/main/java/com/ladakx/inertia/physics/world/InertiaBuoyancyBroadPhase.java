...
final class InertiaBuoyancyBroadPhase {
    private static final int MAX_VERTICAL_SCAN = 16;
    private final World world;

    InertiaBuoyancyBroadPhase(World world) {
        this.world = Objects.requireNonNull(world, "world");
    }

    void findPotentialFluidContacts(Collection<AbstractPhysicsBody> bodies, InertiaBuoyancyDataStore outStore) {
        ...
        for (AbstractPhysicsBody physicsBody : bodies) {
            ...
            InertiaFluidType detectedType = null;
    ...
    private InertiaFluidType toFluidType(Material material) {
        if (material == Material.LAVA) {
            return InertiaFluidType.LAVA;
        }
        return InertiaFluidType.WATER;
    }
}
